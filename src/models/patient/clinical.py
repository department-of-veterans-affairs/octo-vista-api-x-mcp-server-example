...
class Consult(BasePatientModel):
    ...
    @field_validator("provisional_dx", mode="before")
    @classmethod
    def parse_provisional_dx(cls, v):
        """Parse provisional diagnosis"""
        if v is None:
            return None
        if isinstance(v, dict):
            return CodedValue(
                code=v.get("code", ""), name=v.get("name", ""), system=v.get("system")
            )
        return v
    ...
    def status(self) -> ConsultStatus:
        """Get status as enum"""
        try:
            return ConsultStatus(self.status_name.upper())
        except ValueError:
    ...
    def is_active(self) -> bool:
        ...
        return ConsultStatus.is_active(self.status_name.upper())

    @property
    def is_overdue(self) -> bool:
        """Check if consult is overdue based on urgency"""
        if not self.is_active or not self.date_time:
            return False

        days_elapsed = (datetime.now() - self.date_time).days

        if self.urgency.upper() == "STAT":
            return days_elapsed > 1
        elif self.urgency.upper() == "URGENT":
            return days_elapsed > 7
        else:  # Routine
            return days_elapsed > 30

    @property
    def urgency_enum(self) -> Urgency:
        """Get urgency as enum"""
        try:
