...
async def get_patient_procedures_impl(
    ...
    Args:
        ...
        include_modifiers: Include CPT modifiers in response
        group_by_encounter: Group procedures by encounter
        limit: Maximum number of procedures to return

    Returns:
        Dictionary containing procedures data and metadata
    """
    try:
        # Get patient data with proper string types
        resolved_station = station or get_default_station()
        resolved_caller_duz = caller_duz or get_default_duz()
        patient_data = await get_patient_data(
            vista_client=vista_client,
            station=str(resolved_station),
            patient_dfn=patient_dfn,
            caller_duz=str(resolved_caller_duz),
        )
        ...
        if group_by_encounter:
            procedures_data = _group_procedures_by_encounter(
                limited_codes, include_modifiers
            )
        else:
            procedures_data = _format_procedures_list(limited_codes, include_modifiers)
...
def _apply_procedure_filters(
    ...
    if date_from or date_to:
        try:
            from_date = datetime.fromisoformat(date_from) if date_from else datetime.min
            to_date = datetime.fromisoformat(date_to) if date_to else datetime.max
...
def register_get_patient_procedures_tool(mcp, vista_client: BaseVistaClient):
    ...
    async def get_patient_procedures(
        ...
        # Use default station and DUZ
        resolved_station = station or get_default_station()
        resolved_caller_duz = get_default_duz()
